...
//				long[] buffer = new long[BUF_SIZE * numChannels];
	...
	public static double[] readWavFile(String filePath){
		...
		try {
			...
			wavFile.readFrames(buffer, frames);
			wavFile.close();
		} catch (Exception e){
			Log.e("WavFile","Read Wav Error");
		}
		return buffer;
	}

	public static void writeWavFile(String filePath, double[] signal){
		try{
			WavFile wavFileOut = newWavFile(new File(filePath),
					1, signal.length,16, 16000);
			wavFileOut.writeFrames(signal,signal.length);
			wavFileOut.close();
		}catch (Exception e){
			Log.e("WavFile","Write Wav Error");
		}
	}
}
